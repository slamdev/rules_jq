...
def _jq_repo_impl(repository_ctx):
    ...
    file_url = "https://github.com/jqlang/jq/releases/download/jq-{}/jq-{}".format(repository_ctx.attr.jq_version, platform)

    repository_ctx.report_progress("Downloading and extracting JQ toolchain")
    repository_ctx.download(
        url = file_url,
        executable = True,
        output = "jq-{}".format(platform),
    )
